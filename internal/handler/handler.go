...
// AcceptWrite gates WRQs: report?id=, data?id=, debug?id=, and profile?id= are accepted.
...
// data → JSONL telemetry ingest; debug → dbg: response lines;
// profile → opaque profiler blob ingest. Anything else is rejected.
